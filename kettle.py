...
class KettleManager:
	...
	def refresh_options(self):
		...
		for entity in self.game.current_player.hand:
			for option in self.get_options(entity):
				self.options.append(option)

		for entity in self.game.current_player.field:
			for option in self.get_options(entity):
				self.options.append(option)

		for option in self.get_options(self.game.current_player.hero):
			self.options.append(option)

		if self.game.current_player.hero.power:
			for option in self.get_options(self.game.current_player.hero.power):
				self.options.append(option)
	...
	def process_send_option(self, data):
		...
		if option["Type"] == OptionType.END_TURN:
			...
			# CURRENT_PLAYER needs to change before turn.
